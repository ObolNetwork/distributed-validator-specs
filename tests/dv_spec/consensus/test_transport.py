...
from dv_spec.subspecs.consensus.qbft.message import MsgType, QBFTMsg, QBFTConsensusMsg
...
class TestTransport:
    ...
    def test_transport_initialization(self):
        ...
        peers = [
            PeerInfo(peer_idx=i, public_key=b"test_key", peer_id=f"peer_{i}")
            for i in range(4)
        ]
    ...
    def test_set_values(self):
        ...
        peers = [PeerInfo(peer_idx=i, public_key=b"test_key", peer_id=f"peer_{i}") for i in range(4)]
    ...
    def test_sign_message_with_existing_signature_fails(self):
        ...
        with pytest.raises(ValueError, match="Message should not have signature set before signing"):
    ...
    def test_peers_list_access(self):
        ...
        peers = [PeerInfo(peer_idx=i, public_key=b"test_key", peer_id=f"peer_{i}") for i in range(5)]
        transport = Transport(private_key=b"key" * 8, peers=peers)
