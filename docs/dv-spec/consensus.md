...
Both the value hash and the message signing root are `HashRoot(deterministic_proto(...))`. That construction is exact and easy to get subtly wrong, so it is specified separately in [Canonical Encoding and Hashing](hashing.md), with test vectors generated by Charon. What follows is only a summary.
...
- Message signature is computed over `HashRoot(deterministic_proto(QBFTMsg))` with the `signature` field excluded from the encoding
